...
export default function Home() {
  ...
  function formatSerialDate(serial) {
    ...
    return new Date(base.getTime() + ms).toDateString();
  }

  const todaySerial = (() => {
    const base = new Date(Date.UTC(1899, 11, 30));
    const now = new Date();
    return Math.floor((now - base) / (24 * 60 * 60 * 1000));
  })();

  const pastEvents = events.filter(([serial]) => Number(serial) < todaySerial);
  const futureEvents = events.filter(([serial]) => Number(serial) >= todaySerial);

  function renderEventBlocks(data) {
      ...
      return (
        <div key={rowIndex} style={{ background: 'white', padding: '16px', marginBottom: '24px', borderRadius: '10px' }}>
          <div style={{ fontWeight: 'bold', marginBottom: '8px' }}>
            {eventName} ({formatSerialDate(Number(serial))})
          </div>
          <div style={{ marginBottom: '8px', color: '#777' }}>
          ...
          {[
            ...
            vol || phone ? (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                ...
                <a href={`sms:${phone}`} style={buttonStyle}>
                  {phone}
                </a>
              </div>
            ) : null
  ...
  return (
    <div style={{ padding: '16px', fontFamily: 'sans-serif', background: '#f2f2f2' }}>
      ...
      <h3 style={{ color: '#003087' }}>🔵 Upcoming Events</h3>
      {renderEventBlocks(futureEvents)}

      <h3 style={{ color: '#111' }}>⚫ Past Events</h3>
      {renderEventBlocks(pastEvents)}
    </div>
  );
}
